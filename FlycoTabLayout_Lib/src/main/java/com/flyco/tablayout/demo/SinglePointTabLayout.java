 ...
 * 只包含底部圆指示器
 */
public class SinglePointTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    ...
    private float mIndicatorWidth = 10;
    ...
    public SinglePointTabLayout(Context context) {
        this(context, null, 0);
    }

    public SinglePointTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SinglePointTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        mIndicatorCurrentRect.right = (int) right;
        ...
        float indicatorLeft = left + (currentTabView.getWidth() - mIndicatorWidth) / 2;

        mIndicatorCurrentRect.left = (int) indicatorLeft;
        mIndicatorCurrentRect.right = (int) (indicatorLeft + mIndicatorWidth);
    }
...
//        mIndicatorCurrentRect.right = (int) indicatorPoint.right;
    ...
    private void drawUnderline(Canvas canvas, int height, int paddingLeft) {
        if (mIndicatorHeight > 0) {
            ...
            mIndicatorDrawable.setCornerRadius(mIndicatorHeight / 2);
